...
class DatabaseManager:
    ...
    def add_sensor(self, sensor_id: str, sensor_type: int, pin: int, metadata: dict = None):
        cursor = self.conn.cursor()
        try:
            cursor.execute('''
            ...
            if len(parts) > 1:
                ...
                for sensor in sensors:
                    if ':' in sensor:
                        ...
                        self.sensor_inventory[sensor_id] = int(sensor_type)
                        
                        # Update database
                        self.db.add_sensor(sensor_id, int(sensor_type), 0)
